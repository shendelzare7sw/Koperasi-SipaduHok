<x-layouts.app title="Atur Ulang Kata Sandi - Toko Sipaduhok">
    <div class="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p class="mt-2 text-sm leading-6 text-slate-500">Identitas akun telah diverifikasi. Gunakan minimal delapan karakter yang memuat huruf dan angka.</p>
        <form method="POST" action="{{ route('password.update') }}" class="mt-6 space-y-4" data-confirm="Kata sandi akun akan diganti dengan kata sandi baru." data-confirm-title="Simpan kata sandi baru?" data-confirm-button="Ya, simpan">
            @csrf
            <label class="block text-sm font-semibold">Email akun<input type="email" value="{{ $email }}" readonly class="mt-1 w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-500"></label>
