<x-layouts.app title="Identitas Toko">
    <div class="max-w-3xl">
        <p class="text-sm font-extrabold uppercase tracking-widest text-secondary">Pengaturan toko</p>
        <h1 class="mt-1 text-3xl font-black text-slate-900">Identitas & Kontak Toko</h1>
        ...
        <form method="POST" action="{{ route('admin.settings.store.update') }}" class="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7" data-confirm="Identitas dan kontak publik Toko Sipaduhok akan diperbarui." data-confirm-title="Simpan identitas toko?" data-confirm-button="Ya, simpan">
            @csrf @method('PUT')
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <label class="text-sm font-semibold sm:col-span-2">Alamat toko<textarea name="address" rows="4" required class="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3">{{ old('address', $settings['address']) }}</textarea></label>
