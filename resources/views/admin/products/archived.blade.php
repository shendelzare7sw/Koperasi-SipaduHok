<x-layouts.app title="Arsip Produk">
    ...
    <div class="mt-5 overflow-x-auto rounded-2xl border border-slate-200 bg-white">
        <table class="min-w-full text-sm">
            <thead class="bg-slate-100 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th class="p-4">Produk</th><th class="p-4">Kategori</th><th class="p-4 text-center">Foto</th><th class="p-4">Diarsipkan</th><th class="p-4 text-right">Aksi</th></tr></thead>
            <tbody class="divide-y divide-slate-100">
                @forelse($products as $product)
                    <tr class="hover:bg-slate-50">
                        <td class="p-4"><p class="font-bold text-slate-900">{{ $product->name }}</p><p class="text-xs text-slate-500">Rp {{ number_format($product->price, 0, ',', '.') }} · Stok {{ $product->stock }}</p></td>
                        <td class="p-4">{{ $product->categoryLabel() }}</td>
                        <td class="p-4 text-center">{{ $product->images_count }}</td>
                        <td class="p-4 text-xs text-slate-500">{{ $product->deleted_at?->format('d/m/Y H:i') }}</td>
                ...
                @empty
                    <tr><td colspan="5" class="p-12 text-center"><i class="fas fa-box-open text-4xl text-slate-300" aria-hidden="true"></i><p class="mt-3 font-bold text-slate-700">Arsip produk kosong.</p></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
