<x-layouts.app title="Pembayaran {{ $order->invoice_number }}">
    ...
    <div class="mx-auto max-w-2xl">
        <a href="{{ route('orders.show', $order) }}" class="text-sm font-bold text-primary hover:text-secondary">← Kembali ke detail pesanan</a>

        <section class="mt-5 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
            <div class="bg-gradient-to-br from-primary to-secondary px-6 py-8 text-center text-white sm:px-10">
                <div class="mx-auto grid h-16 w-16 place-items-center overflow-hidden rounded-2xl bg-white p-1.5">
