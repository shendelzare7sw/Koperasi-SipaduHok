<aside class="h-fit min-w-0 rounded-2xl bg-gradient-to-b from-primary to-primary-dark p-3 text-white shadow-lg shadow-primary/15 lg:sticky lg:top-6 lg:p-4">
    ...
    <nav class="flex gap-2 overflow-x-auto lg:mt-3 lg:grid lg:gap-1" aria-label="Menu admin koperasi">
        ...
        <a href="{{ route('admin.buyers.index') }}" class="whitespace-nowrap rounded-xl px-3 py-2.5 text-sm font-bold {{ request()->routeIs('admin.buyers.*') ? $activeClass : $idleClass }}">Akun Pembeli</a>
        ...
        <a href="{{ route('admin.settings.store.edit') }}" class="whitespace-nowrap rounded-xl px-3 py-2.5 text-sm font-bold {{ request()->routeIs('admin.settings.*') ? $activeClass : $idleClass }}">Identitas Koperasi</a>
