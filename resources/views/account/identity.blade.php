<x-layouts.app title="Verifikasi KTP - Koperasi Sipaduhok">
    @php
        ...
        <div class="mt-6 grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            @if($verified)
                <section class="h-fit rounded-2xl border border-emerald-200 bg-white p-6 shadow-sm sm:p-8">
                    <div class="flex items-start gap-4"><span class="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-emerald-100 text-emerald-600"><i class="fas fa-shield-halved text-xl" aria-hidden="true"></i></span><div><h2 class="text-xl font-black text-slate-900">Verifikasi selesai</h2><p class="mt-2 text-sm leading-6 text-slate-600">Untuk mencegah perubahan identitas tanpa pemeriksaan, data KTP yang sudah disetujui tidak dapat diganti dari akun pembeli. Hubungi admin koperasi bila terdapat kesalahan data.</p><a href="{{ route('catalog.index') }}" class="mt-5 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 font-bold text-white hover:bg-secondary"><i class="fas fa-store" aria-hidden="true"></i>Mulai Belanja</a></div></div>
                </section>
            @else
