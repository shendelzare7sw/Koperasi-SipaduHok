    ...
    <div class="mx-auto max-w-2xl">
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/60">
            <div class="bg-gradient-to-br from-primary to-secondary px-6 py-8 text-center text-white sm:px-10">
                ...
                <p class="mt-2 text-sm text-blue-50">Lanjutkan ke halaman pembayaran yang aman untuk menyelesaikan pesanan.</p>
            </div>
            ...
            <div class="p-6 sm:p-10">
                <dl class="space-y-4 rounded-2xl bg-slate-50 p-5 text-sm">
                    ...
                    @if($order->gateway_total && $order->gateway_total !== $order->total)
                        <div class="flex justify-between gap-4 border-t border-slate-200 pt-4 text-lg"><dt class="font-black">Total pembayaran</dt><dd class="font-black text-primary">Rp {{ number_format($order->gateway_total, 0, ',', '.') }}</dd></div>
                    @endif
                ...
                @if($order->payment_url)
                    <a href="{{ $order->payment_url }}" rel="noreferrer" class="mt-6 block w-full rounded-xl bg-primary px-5 py-4 text-center font-black text-white shadow-lg shadow-primary/20 transition hover:bg-secondary"><i class="fas fa-arrow-up-right-from-square mr-2" aria-hidden="true"></i>Selesaikan Pembayaran</a>
                @else
